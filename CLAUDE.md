...
- **static/app.js** — Plain ES5-ish JS. No build step. Uses `XMLHttpRequest` for same-origin API calls (WebKit 533 compat). Sends full chat history on each request. Settings panel opens automatically if no `base_url` is configured.
...
- **Chat uses status indicator**: The gateway streams from the upstream LLM (`stream: True`) for cancellation support and efficiency. The server sends NDJSON status events to the client (Sending… → Thinking… → Generating…), and the full response is rendered once when complete. No progressive text updates — the e-ink display only refreshes for status changes and the final result.
